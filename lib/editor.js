var editor = (function() {
  ...
  var Mapr = (function() {
    ...
    function setupCanvii() {
      ...
      if (!canvas) {
        ...
        function handleClick() {
          var mouse_cell_x = Math.floor( (mouse.x) / (cw*zoom) );
          var mouse_cell_y = Math.floor( (mouse.y) / (ch*zoom) );
    ...
    function newMap(w, h) {
      // create our data
      var map = new MapSegment({w: w, h: h});
      console.log(map);
      for (var y = 0; y < h; y++) {
        map.cells[y] = [];
        for (var x = 0; x < w; x++) {
        ...
        for (var x = 0; x < current_map.w; x++) {
          if (current_map.cells[y][x].tile >= 0) {
            if (Spritr.sprites.length > current_map.cells[y][x].tile) {
              ...
              map_ctx.drawImage(Spritr.image, sprite.x, sprite.y, sprite.w, sprite.h, x*cw, y*ch, sprite.w, sprite.h);
            }
          }
        }
      }
    ...
    function renderCanvas() {
      ...
      var mouse_cell_x = Math.floor( (mouse.x) / (cw*zoom) );
      var mouse_cell_y = Math.floor( (mouse.y) / (ch*zoom) );
      ctx.strokeRect((mouse_cell_x*cw*zoom)+0.5, (mouse_cell_y*ch*zoom)+0.5, cw*zoom, ch*zoom);
      ctx.globalAlpha = 1.0;
    }
      ...
      if (in_border.checked) {
        ...
        for (var y = 0; y < current_map.h; y++) {
          for (var x = 0; x < current_map.w; x++) {
            ...
            overlay_ctx.strokeRect((x*cw*zoom)+0.5, (y*ch*zoom)+0.5, cw*zoom, ch*zoom);
          }
        }
    ...
    function saveMap() {
      if (!current_map) return;
      var map = new MapSegment(current_map);
      console.log(map);
      saveData('segment.json', JSON.stringify(map), 'text/javascript');
      //console.log(JSON.stringify(map));
      //map.compress();
      //console.log(JSON.stringify(map));
    }
    function loadMap(data) {
      var map = new MapSegment(data);
      maps.push(map);
      current_map = map;
      // create our view
      setupCanvii();
  ...
  function attachButtons() {
    ...
    var h_load = document.createElement('input');
    h_load.type = 'file';
    h_load.addEventListener('change', function(e) {
      if (e.target.files.length <= 0) return;
      var reader = new FileReader;
      reader.onload = function(e) {
        Mapr.loadMap(JSON.parse(e.target.result));
      };
      reader.readAsText(e.target.files[0]);
    });
