...
var s2d = (function() {
  ...
  var g_can_sync = {};
  var g_sprite_data = {};
  var g_sprite_data_packed = false;
    ...
    animations.addInput('button', 'add', '+', function(cb) {
      ...
      sets.addInput('button', 'add', '+', function(cb) {
        ...
        frames.addInput('button', 'add', '+', function(cb) {
          ...
          frame.addInput('number', 't', '', function(){syncData(name)});
    ...
    for (i in tree.branches[1].branches) {
      ...
      for (i in A.branches[1].branches) {
        ...
        for (i in S.branches[2].branches) {
          ...
          frame.t = parseInt(F.getInput('t').value || sprite_data.A[A_name].S[S_name].C.t || 100);
  ...
  function syncDataToTree(name) {
    ...
    if (!sprite_data.C) sprite_data.C = {};
    if (!sprite_data.C.x) sprite_data.C.x = '';
    if (!sprite_data.C.y) sprite_data.C.y = '';
    if (!sprite_data.C.w) sprite_data.C.w = '';
    if (!sprite_data.C.h) sprite_data.C.h = '';
    if (!sprite_data.C.t) sprite_data.C.t = '';
  ...
  function playAnim() {
    ...
    frame_draw.do_draw = 0;
    s_frame = getFrame(sprite_data, g_anim_name, g_set_name, ++g_frame_number);
  ...
  function loadData(name, data_) {
    ...
    if (obj.packed) {
      delete obj.packed;
      g_sprite_data_packed = true;
      for (name in obj) {
        g_sprite_data[name] = obj[name];
        syncDataToTree(name);
      }
    } else {
      g_sprite_data_packed = false;
      g_sprite_data[name] = obj;
      syncDataToTree(name);
    }
  }
  function saveDataFile(name) {
    stopAnim();
    var a = document.createElement('a');
    var l_data = {};
    // FIXME: we should probably clone g_sprite_data[...].
    if (!name) {
      name = 'packed'; // FIXME.
      l_data.packed = true;
      for (var name in g_sprite_data) {
        disableSync(name);
        l_data[name] = g_sprite_data[name];
        enableSync(name);
      }
    } else {
      disableSync(name);
      l_data = g_sprite_data[name];
      enableSync(name);
    }
    // TODO: optimizeData(l_data);
  ...
  function gogogo() {
    ...
    var e_save_data = document.getElementById('save_data');
    e_save_data.addEventListener('click', function() {
      saveDataFile(null);
    });
