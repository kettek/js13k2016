...
var s2d = (function() {
  ...
  function createPNode(parent) {
    ...
    pnode.addText('Animation: ');
    pnode.addInput('text', 'anim', '', function(cb) {
    });
    pnode.addText('Offset: ');
  ...
  function createPTree() {
    ...
    ptree = new Branch(document.getElementById('ptree'), 'Animations: ');
    ptree.addText(' Set: ');
    ptree.addInput('text', 'set', '', function(cb) {
    });
    ptree.addInput('button', 'add', '+', function(cb) {
      createPNode(ptree);
    });
  }
    ...
    animations.addInput('button', 'add', '+', function(cb) {
      ...
      sets.addInput('button', 'add', '+', function(cb) {
        ...
        points.addInput('button', 'add', '+', function(cb) {
          ...
          point.addInput('text', 'name', '', syncData);
          ...
          point.addInput('button', 'add', '+', function(cb) {
            ...
            pframe.addInput('number', 'x', '', syncData).addInput('number', 'y', '', syncData);
  ...
  function drawPreview() {
    ...
    if (img_loaded) {
      ...
      if (s.P) {
        if (s.P["offset"]) {
          if (s.P["offset"][frame]) {
            o_x = s.P["offset"][frame][0];
            o_y = s.P["offset"][frame][1];
          }
        }
      }
  ...
  function saveDataFile() {
    ...
    var l_data = optimizeData(sprite_data);
